<?php
class Upload_bl_model extends CI_model
{
	...
	public function shippingdata()
	 ...
	 { 	
		$this->db->where('id',$id);
		$updateid= $this->db->update('upload_bl',$data);
		return $updateid;
	 }
	...
	public function insert_bl($data){
		...
		$id = $this->db->insert('upload_bl',$data);
		 
		return $this->db->insert_id();
	}
	public function get_export_data()
	{
		$q = "SELECT export_invoice_id,export_invoice_no from  tbl_export_invoice as mst where status=0 and invoice_date >='2020-04-01'";
	...
	public function get_upload_bl($id){
		$q = $this->db->where('id',$id);
		$q = $this->db->get('upload_bl');
		 return $q->row();
		 
	}
public function update_upload_bl($data,$uid){
	
	$this->db->where('id',$uid);
	$q=$this->db->update('upload_bl',$data);
	return $q;
