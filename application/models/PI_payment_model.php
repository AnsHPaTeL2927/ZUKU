<?php
class PI_payment_model extends CI_model
{
	...
	public function delete_trn_payment($po_payment_id)
	{
		...
		$q1 = $this->db->where('table_name','tbl_pi_advance_payment');
		$q1 = $this->db->where('table_trn_id',$pi_advance_payment_id);
		$q1 = $this->db->delete('tbl_receive_payment_part_trn');
	 	return $q;
	}
	public function update_advance($data,$pi_advance_payment_id)
	{
		 
		$q = $this->db->where('pi_advance_payment_id',$pi_advance_payment_id);	
	...
	public function get_advance_payment_data($id)
	{
		...
		$q=$this->db->get('tbl_pi_advance_payment');
		$data = $q->row();
