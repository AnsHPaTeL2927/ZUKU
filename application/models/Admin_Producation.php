...
class Admin_Producation extends CI_model
{
	...
	public function get_container_wise($production_mst_id)
{
    return $this->db
        ->where('production_mst_id', $production_mst_id)
        ->get('tbl_production_mst')
        ->result();
}
...
public function get_all_cofirm_pi($customer_id, $start_date, $end_date)
{
    ...
    $where = ' AND mst.performa_date BETWEEN "'.$start.'" AND "'.$end.'"';
    ...
    $q = '
        SELECT 
            ...
            -- Shows all producation_no like: P001, P003, P005
            GROUP_CONCAT(pmst.producation_no SEPARATOR ", ") AS producation_no

        FROM tbl_performa_invoice AS mst

        INNER JOIN customer_detail AS con 
            ON con.id = mst.consigne_id
        
        LEFT JOIN tbl_production_mst AS pmst
            ON pmst.performa_invoice_id = mst.performa_invoice_id

        WHERE mst.confirm_status = 1
        '.$where.'

        GROUP BY mst.performa_invoice_id   -- important to avoid duplicates

        ORDER BY mst.performa_invoice_id DESC
    ';
	...
	public function delete_manageproducation($id)
	{
	 	$this->db->where('production_mst_id', $id);
		return $this->db->delete('tbl_pallet_box_entry');
	}
	...
	public function get_order_sheet($size,$finish_id,$cust_id)
	{
		 $where = '';
		 if(!empty($size))
		 {
		 ...
		 if(!empty($finish_id))
		 {
		  ...
		  // if(!empty($finish_id))
		 // {
			 // $where .= ' and consign.id ='.$finish_id;
		 // } 
		...
		INNER join tbl_performa_invoice as pi on pi.performa_invoice_id = mst.performa_invoice_id
		INNER join customer_detail consign on consign.id = pi.consigne_id
		INNER join tbl_performa_trn as performa_trn on performa_trn.performa_trn_id = packing.performa_trn_id
		INNER join tbl_product as pro on pro.product_id = performa_trn.product_id 
		WHERE  mst.status = 0 '.$where.' and performa_trn.product_id  !=0 group by model.packing_model_id,finish.finish_id';
		$q_con = $this->db->query($qry);
