<?php
class Cha_master_model extends CI_model
{
	...
	public function bank_select()
	 {
		$q= $this->db->select("mst.*")
			 ...
			 //->where('mst.status',"0")
			->where('mst.id not in (select id from tbl_cha_master where id = mst.id ) and mst.id !=',0)
			 ->get();
	...
	public function supplier_select(){
		...
		$q=$this->db->get('tbl_cha_master');
		return $q->result();
	}
	public function supplierinsert($data)
	{
		$q = $this->db->insert('tbl_cha_master',$data);
	...
	{
		$this->db->where('id', $id); 
		return $this->db->update('tbl_cha_master',$data); 
	}
	public function delete_supplier($id){
		
		$this->db->where('id',$id);
		return $this->db->delete('tbl_cha_master');
	}
	...
	public function check_epcg_data($series_name,$supplier_id)
	{
		...
		$q= $this->db->get("tbl_cha_contact");
		return $q->row();
	}
	public function insert_epcg_data($data)
	{
		$q = $this->db->insert('tbl_cha_contact',$data);
		return $this->db->insert_id();
	}
	public function get_epcg_data($id)
	{
		$q = $this->db->select("mst.*")
			->from("tbl_cha_contact as mst")
			->where("cha_id",$id)
			->get();
		// echo $this->db->last_query();
		return $q->result();
	...
	public function update_epcgdata($data,$id)
	{
		$this->db->where('contact_id', $id); 
		 return $this->db->update('tbl_cha_contact',$data); 
	}
	public function deleteepcgrecord($id){
		
		$this->db->where('contact_id',$id);
		return $this->db->delete('tbl_cha_contact');
	}
}
