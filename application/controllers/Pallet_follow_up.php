...
class Pallet_follow_up extends CI_controller
{
	...
	function index()
	{
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		{
			...
			$this->load->view('admin/pallet_follow_up',$data);
		}
		else
		{
			redirect(base_url().'');
		 ...
		 $invoicedate = explode(" - ",$this->input->post('daterange'));
			$str = ' <table class="table table-bordered table-hover display" id="datatable" width="100%">
						<thead>
							...
							<th class="text-center" width="9%">PI No</th>
							<th class="text-center" width="9%">Production Date</th>
							<th class="text-center" width="8%">No of Pallet</th>
							<th class="text-center" width="8%">Pallet / Box</th>
							<th class="text-center" width="10%">Size</th>
						 	<th class="text-center" width="8%">Pallet Order</th>
						 	<th class="text-center" width="8%">Pellet Type</th>
			...
			$str .= '<tbody>
					...
					if(!empty($row->producation_no))	
					{
						...
						$follow_date = !empty($get_latest_followup)?date('d/m/Y H:i A',strtotime($get_latest_followup->follow_date)):"";
						$str .= '<td  class="text-center">'.$get_latest_followup->remarks.'</td>
								...
								<a class="tooltips  btn btn-primary" data-toggle="tooltip" data-title="" href="javascript:;" onclick="add_fllow_up('.$row->production_mst_id.',&quot;'.$row->producation_no.'&quot;)"   ><i class="fa fa-plus"></i></a>
	 ...
	 public function fetch_followup_record()
	  {
	 	$performa_invoice_id = $this->input->get('performa_invoice_id');
				...
				$aColumns = array('pi_followup_id', 'invoice.invoice_no','follow_date','type.follow_up_type','mst.remarks');
				...
				foreach($sqlReturn['data'] as $row)
				{
					...
					$row_data[] = date('d/m/Y h:i A',strtotime($row->follow_date));
