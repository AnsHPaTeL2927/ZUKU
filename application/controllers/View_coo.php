...
class View_coo extends CI_controller
{
		...
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		 {
				...
				$html_data				= $this->custinvoice->get_coo_html($id);
			...
			$this->load->view('admin/view_coo',$v);
		}
		else
		{
			redirect(base_url().'');
		...
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		 {
			...
			$get_bldraft = $this->draft->get_bldraft_data($id);
			$data		= $this->draft->exportinvoice_data($get_bldraft->export_invoice_id);
	 	 	$datap		= $this->draft->get_export_trn_data($get_bldraft->export_invoice_id);
	...
	function viewpdf()
	{
		...
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		{
			...
			$this->load->view('admin/coo_pdf',$v);
		}
		else
		{
			redirect(base_url().'');
		...
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		 {
			...
			$this->load->view('admin/coo_doc',$v);
		}
		else
		{
			redirect(base_url().'');
