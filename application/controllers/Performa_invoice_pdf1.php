...
class Performa_invoice_pdf1 extends CI_controller
{
	...
	function index($id)
	{
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		{
			...
			$html_data	 	= 	$this->pinv->get_invoice_html($id,'performa_invoice_pdf1');
			...
			$this->load->view('admin/performa_invoice_pdf1',$v);
		}
		else
		{
			redirect(base_url().'');
