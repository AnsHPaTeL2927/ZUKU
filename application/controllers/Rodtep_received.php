...
class Rodtep_received extends CI_controller{
		...
		public function index($m="")
		{
			if(!empty($this->session->id)  && $this->session->title == TITLE)
			{
				...
				$data['menu_data']	 	= $this->menu->usermain_menu($this->session->usertype_id);	
			 	$this->load->view('admin/rodtep_received',$data);
			}
			else
			{
				redirect(base_url().'');
		...
		{
			$data1 = array(
				"rodtep_received_date" 			=>	date('Y-m-d',strtotime($this->input->post('payment_date'))),
		...
		public function fetch_record()
		{
			...
			$aColumns = array('mst.id', 'invoice.export_invoice_no','invoice.invoice_date','mst.shipping_bill_no','mst.date','mst.rodtep_amount','mst.exchange_rate','mst.rodtep_payment','mst.rodtep_received_date','mst.cdate','mst.status');
