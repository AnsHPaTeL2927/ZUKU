		...
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		{
			...
			$menu_data			= $this->menu->usermain_menu($this->session->usertype_id);	
			 $data = array(
				...
				'producation_data'	=> $this->producation->get_order_sheet($size,$finish_id)
	...
	public function get_order_sheet()
	{
		...
		$str = '<div class="table-responsive">
					<table class="table table-bordered  display" id="datatable" width="100%">
						...
						$order_sheet_data = $this->producation->get_order_sheet($size,$finish_id);
			 		$sr =1;
						...
						<thead>
						 ...
						 for($i=0;$i<count($product_id);$i++)
						 {
							 ...
							 $str .= '<tr>
										...
										<td style="text-align:center">
												<input type="text" name="producation_box[]" id="producation_box'.$no.'" class="form-control"/>
