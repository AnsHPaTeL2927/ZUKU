...
class Doc_pending extends CI_controller{
	...
	public function __construct(){
 	...
 	public function index()
 	{
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		{
			...
			$this->load->view('admin/doc_pending',$data);	
		}
		else
		{
			redirect(base_url().'');
	...
	public function view_pdf()
 	{
		$this->load->view('admin/doc_pending_pdf');
	}
	public function view_report()
	{
		...
		$resultdata		=	$this->order->get_doc_pending($customer_id,$finish_id,$size_id,$pallet_type,$invoicedate[0],$invoicedate[1]);
