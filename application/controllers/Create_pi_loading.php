...
class Create_pi_loading extends CI_controller
{
	...
	public function download() 
	{
		...
		// read file contents
		$data = file_get_contents(base_url('./upload/csv/sample_demo.csv'));
		force_download($data);
	}
	...
	function index($id)
	{
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		{
			...
			$producation_data 		= 	$this->pinv->get_producation_data1($id,0);
			//print_r($producation_data);
			//exit();
	...
	function set_auto_loading_plan()
	{
		...
		$producation_data 		= $this->pinv->get_producation_data1($performa_invoice_id,0);  
	...
	public function remaining_pi()
{
    $performa_invoice_id = $this->input->post('performa_invoice_id');
    $remainpi_detail = $this->pinv->remain_data_detail($performa_invoice_id,0,0,0);
	...
	$str = '
			 ...
			 ';
					$no=1;
					$conarray = array();
		foreach($remainpi_detail as $row)
		{
			...
			$str .= '<tr>
						...
						{	
							...
							$str .= '<td rowspan="'.$rowspan.'" class="text-center">'.$row->pending.'</td>';
				 	...
				 	$str .= '
						<td class="text-center">
							<a class="tooltips btn btn-info" data-toggle="tooltip" data-title="Print" href="'.base_url().'create_pi_loading/pendingconprint/'.$performa_invoice_id.'/'.$row->supplier_id.'/'.$row->export_time.'" data-original-title="" title=""> Print</a>
						</td>
					</tr>';
					$no++;
		}			
	...
	function pendingconprint($performa_invoice_id,$supplier_id,$export_time)
	{
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		 {
			$company			=	$this->pinv->company_select();
			
			$set_container		= 	$this->pinv->loading_data_pending($performa_invoice_id,$supplier_id,$export_time);
			...
			$this->load->view('admin/pending_print_loading',$v);
		}
		else
		{
			redirect(base_url().'');
		...
		if($_SERVER['REQUEST_METHOD'] === 'POST')
			{
				...
				if(strtolower($extension) == 'csv')
				{
					...
					$v['csverror'] = '<code>Invalid file! please import .csv file.</code>';
				}
			}
	...
	public function copy_containter()
	{
		...
		$container 	  				= $this->input->post('container');
	 	$get_data					= $this->pinv->producation_trn_data1($production_mst_id);
		...
		foreach($get_data as $packing)
		{
			...
			$str .= '<td>
						<input type="text" name="batch_no[]" id="batch_no'.$packing->production_trn_id.'" value="'.$packing->pro_batch.'"   />
					</td>
					<td>
						<input type="text" name="shade_no[]" id="shade_no'.$packing->production_trn_id.'"  value="'.$packing->pro_shade.'"  />
					</td>
	...
	public function copy_multiple_containter()
	{
		...
		$str = '<table class="table table-bordered">
					<tr>
						...
						<td class="text-center" width="10%">Pallet</td>
						<td class="text-center" width="10%">Boxes</td>
						<td class="text-center" width="14%">Sqm</td>
						<td class="text-center" width="8%">Unit</td>
						<td class="text-center" width="8%">Quantity</td>
						<td class="text-center" width="10%">Batch No</td>
						<td class="text-center" width="10%">Shade No</td>
					</tr>';
