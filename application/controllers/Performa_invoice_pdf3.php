...
class Performa_invoice_pdf3 extends CI_controller
{
	...
	function index($id){
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		{
			...
			$html_data	 	= 	$this->pinv->get_invoice_html($id,'performa_invoice_pdf3');
			...
			$this->load->view('admin/performa_invoice_pdf3',$v);
		}
		else
		{
			redirect(base_url().'');
