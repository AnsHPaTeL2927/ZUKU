...
class Create_producation extends CI_controller
{
	...
	public function copy_containter()
	{
		...
		$performa_invoice_id	 = $this->input->post('performa_invoice_id');
		 ...
		 $producationno = $this->input->post('producation_no');
		 $productionmst_id = empty($this->input->post('production_mst_id')) ? 0 : $this->input->post('production_mst_id');
		 $check_production_mst = $this->db->select('producation_no')->where('production_mst_id',)->get('tbl_production_mst')->row_array();
		 
		 if(empty($this->input->post('production_mst_id')) || !empty($check_production_mst) && $check_production_mst['producation_no'] != $producationno)
		 {
