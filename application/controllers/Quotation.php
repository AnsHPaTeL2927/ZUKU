...
class Quotation extends CI_controller
{
	...
	function index()
	{
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		{
			...
			$exporter			= $this->quotation->select_exporter();
			$consign			= $this->quotation->select_consigner();
			$selectinvoicetype 	= $this->quotation->selectinvoicetype(6); 
			...
			$v = array(
				...
				'menu_data'			=> $menu_data,
				'mode'			  	=> 'Add',
				'quotationtype'	  	=> $selectquotationtype,
				...
				'termsdata'		  	=> $this->quotation->gettermsdata(),
				'invoicetype'		=> $this->quotation->selectinvoicetype(6),
				 'all_bank'		  	=> $this->bd->b_select()
			);
		$this->load->view('admin/quotation',$v);
		}
		else
		{
			redirect(base_url().'');
	...
	function manage()
	{
		$check_box_design = $this->quotation->check_terms_update($this->input->post('payment_terms'));
		if(empty($check_box_design ))
		{
	 	...
	 	$data = array(
			...
			'estimate_no' 				=> $this->input->post('invoice_no'),
			...
			'quotation_date' 			=> date('Y-m-d',strtotime($this->input->post('date'))),
			...
			{				
				...
				{
					$pdata = $this->quotation->terms_insert($data1);
					$rdata = $this->quotation->quotation_insert($data);
					if($rdata)
					{	
						 
						$row['res'] = 1;
						$row['quotationid'] = $rdata;
					}
				}
				else
				{
			...
			else if(strtolower($this->input->post('mode'))=="edit")
			{
				...
				$rs = $this->quotation->update_quotation($data,$id);
				if($rs)
				{	
					$row['res'] = 3;
					$row['quotationid'] = $id;
 	...
 	public function form_edit($id)
	{
		...
		$menu_data			= $this->menu->usermain_menu($this->session->usertype_id);	
		$this->load->model('admin_bank_detail','bd');	
		$consign_other=$this->quotation->other_consigner($data->consigne_id);
		$all_bank		 	= $this->bd->b_select();
		...
		$v = array(
			...
			'currencydata'=>$this->admin_currency_list->getcurrencydata(),
			'termsdata'	=>$this->quotation->gettermsdata(),
			'notifydata'	=>$this->quotation->customerallconsignerdetail($data->consigne_id),
			'all_bank'		  => $this->bd->b_select(),
	...
	public function selectcompnay()
	{
		...
		if($type=='consigner')
		{
			...
			$fetchdata = $this->quotation->customerdetail($id);
			$fetchdata1 = $this->quotation->customerallconsignerdetail($id);
			...
			echo json_encode(array('data1'=>$fetchdata,'data2'=>$notify_html));
		}
		else if($type=='consignerid')
		{
			$fetchdata1 = $this->quotation->customerconsignerdetail($id);
