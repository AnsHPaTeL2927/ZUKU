...
class Admin extends CI_Controller{
	...
	public function login()
	{
		...
		$newdata = array(
			...
			'id' 			=> 1,
			'usertype_id'  	=> 1,
			'title' 		=> 'ITACA CERAMIC PRIVATE LIMITED',
			'email'     	=> base_url(),
		...
		// 		$targetDate = $checkserial_no->cdate;
		// 		$targetDateObj = new DateTime($targetDate);
		// 		$currentDate = new DateTime(); // Current date
		// 		// Calculate the difference
		// 		$diff = $currentDate->diff($targetDateObj);
		// 		$renew_key_log['renew_key_verify'] = $diff->days <= 365 ? 1 : 0;
		// 		$renew_key_log['pending_day'] = 365 - $diff->days;
		// 		$row['renew_key_verify'] = $diff->days <= 365 ? '' : 'https://www.google.com/maps';
		// 		$checksub = (CONFIGURATION == 0) ? 1 : $this->admin_login->checkp($data);

		// 		if (!empty($checksub) && $diff->days <= 365) {
		// 			$checkdata = $this->admin_login->login($data);

		// 			if (!empty($checkdata)) {
		// 				// Add Log
		// 				$this->admin_login->add_user_key_verification_log($renew_key_log);
