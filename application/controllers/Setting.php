...
class Setting extends CI_controller
{
	...
	public function index()
	{
		if(!empty($this->session->id)  && $this->session->title == TITLE)
		{
			...
			$resultdata['setting_data_pi'] = $this->setting->setting_data_pi(1);
			...
			$resultdata['menu_data']	 	= $this->menu->usermain_menu($this->session->usertype_id);	
		 	$this->load->view('admin/setting',$resultdata);
		}
		else
		{
			redirect(base_url().'');
	...
	public function use_onoff_pi()
	{
		$checked=$this->input->post('value');
		$control=$this->input->post('control');
		$deleteid=$this->setting->update_onoff_pi($checked,$control);
		if($deleteid)
		{
			$row['res'] = 1;
			$_SESSION['updatedinsetting'] = 5;
		}
		else{
			$row['res'] = 0;
		}
