					...
					<div class="row">
						<div class="col-sm-12">
							<ol class="breadcrumb">
								...
								<li class="active">
									<a href="#"> Master&nbsp;&nbsp; </a>   /  &nbsp;&nbsp;Fumigation 
								</li>
							</ol>
							<div class="page-header">
							<h3>Fumigation<p align="right">
								...
								<div class="panel-body">
									<form role="form" class="form-horizontal" action="javascript:;" method="post" enctype="multipart/form-data" name="fumigation_add" id="fumigation_add">
									 	<div class="form-group">
											...
											<div class="col-sm-12">
												<input type="text" placeholder="Fumigation" id="fumigation_name" class="form-control" name="fumigation_name" />
											</div>
										</div>
										...
										<div class="modal-dialog">
											<div class="modal-content">
												<div class="modal-header">
													...
													<h4 class="modal-title">How To Manage Fumigation</h4>
												</div>
												<div class="modal-body">
													<iframe id="cartoonVideo" width="560" height="315" src="https://www.youtube.com/embed/5pPHT8CcaJM?rel=0&autoplay=0"" frameborder="0" allowfullscreen></iframe>
												</div>
											</div>
										</div>
									</div>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Edit Fumigation</h4>
            </div>
			 <form class="form-horizontal askform" action="javascript:;"  method="post" name="edit_fumigation" id="edit_fumigation">
				<div class="modal-body">
					 <div class="form-group">
					 	...
					 	<div class="col-sm-12">
					 		<input type="text" placeholder="Fumigation" id="edit_fumigation_name" class="form-control" name="edit_fumigation_name" value="" >
					 	</div>
					</div>
		 		   </div>
            <div class="modal-footer">
				...
				<input type="hidden" name="editfumigationname" id="editfumigationname" />
			 </form>
        </div>
    </div>
</div>
...
$(document).ready(function(){
    ...
    $("#myModal1").on('hide.bs.modal1', function(){
        $("#cartoonVideo").attr('src', '');
    });
    
    /* Assign the initially stored url back to the iframe src
    attribute when modal is displayed again */
    $("#myModal1").on('show.bs.modal1', function(){
...
function load_data_table()
{
	datatable = $("#datatable").dataTable({
			...
			"iDisplayLength": 50,
			"sAjaxSource": root+'fumigation_list/fetch_record/',
...
{
	main_delete(deleleid,'fumigation_list/deleterecord','fumigation_list')
}
...
$("#edit_fumigation").submit(function(event) {
	...
	$.ajax({
            type: "post",
            url: 	root+'fumigation_list/manage',
            data: postData,
			processData: false,
			contentType: false,
			cache: false,
            success: function(responseData) {
               console.log(responseData);
			    var obj= JSON.parse(responseData);
				$(".loader").hide();
				if(obj.res==1 || $("#edit_fumigation_name").val() == obj.fumigation_name)
			   {
				    $("#myModal").modal('hide');
				...
				{
					unblock_page("info","Famigation is already exist.");
				}
			   else
			   {
				    unblock_page("error","Something Wrong.") 
...
function editfumigation(id)
{
	block_page();
     $.ajax({ 
              type: "POST", 
              url: root+"fumigation_list/fetchdata",
              data: {"id": id}, 
              success: function (response) { 
				   ...
				   $("#myModal").modal({
					 ...
					 $("#eid").val(id);
					 $("#edit_fumigation_name").val(obj.fumigation_name);
					 $("#editfumigationname").val(obj.fumigation_name);
			 	 	unblock_page("",""); 
                  }
