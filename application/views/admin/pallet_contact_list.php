					...
					<div class="row">
						<div class="col-sm-12">
							<ol class="breadcrumb">
								...
								<li class="active">
									<a href="<?=base_url()?>pallet_order_party"> PALLET MASTER </a>
								</li>
								<li class="active">
									<?=$form?> List
								</li>
							...
							<h3><?=$form?> List Of <?=$pallet_data->party_name?> </h3>
						</div>
					</div>
					</div>
					<div class="row">
								...
								<div class="panel-body">
									<form role="form" class="form-horizontal" action="javascript:;" method="post" enctype="multipart/form-data" name="epcg_add" id="epcg_add">
										...
										<div class="form-group">
											...
											<div class="col-sm-12">
												<input type="text" placeholder="Contact Number" id="Contact_Number" class="form-control" name="Contact_Number" value="" autocomplete="off" required title="Enter Proper Number" onkeypress="return isNumber(event)">
											</div>
										</div>
										<div class="form-group col-md-12" >
											...
											<a href="<?=base_url()?>pallet_order_party" class="btn btn-danger">
												Cancel
											</a>
										</div>
            ...
            <div class="modal-header">
			 ...
			 <form class="form-horizontal askform" action="javascript:;"  method="post" name="edit_form" id="edit_form">
				...
				<div class="form-group">
					...
					<div class="col-sm-12">
						<input type="text" placeholder="Contact Name" id="Contact_Name_details" class="form-control" name="Contact_Name" value="" required title="Contact Name" autocomplete="off" onkeypress="return isNumber(event)">
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-12 control-label" for="form-field-1">
...
{
	main_delete(deleleid,'pallet_contact_list/deleterecord','pallet_contact_list/index/'+<?=$supplier_id?>)
}
$("#epcg_add").validate({
		rules: {
			epcg_no: {
...
$("#epcg_add").submit(function(event) {
	...
	$.ajax({
            type: "post",
            url : root+'pallet_contact_list/manage',
            data: postData,
			processData: false,
			contentType: false,
			cache: false,
            success: function(responseData) {
               console.log(responseData);
				...
				if(obj.res==1)
			    {
				   ...
				   setTimeout(function(){ window.location=root+'pallet_contact_list/index/'+<?=$supplier_id?> },1500);
				}
				else if(obj.res==2)
			    {
...
$("#edit_form").submit(function(event) {
	...
	$.ajax({
            type: "post",
            url: 	root+'pallet_contact_list/edit_record',
            data: postData,
			processData: false,
			contentType: false,
			cache: false,
            success: function(responseData) {
               console.log(responseData);
			    var obj= JSON.parse(responseData);
				$(".loader").hide();
				if(obj.res==1 || $("#Contact_Name_details").val() == obj.editpalletmode)
			   {
				    $("#myModal").modal('hide');
				...
				{
					unblock_page("info","Contact Name already exist");
...
function edit_product(supplie_epcg_id)
{
	block_page();
     $.ajax({ 
              type: "POST", 
              url: root+"pallet_contact_list/fetchdata",
              ...
              success: function (response) { 
				   ...
				   $("#myModal").modal({
					...
					$("#editpalletmode").val(obj.Contact_Name);
					unblock_page("",""); 
                  }
