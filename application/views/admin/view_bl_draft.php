...
$export 					= ($invoicedata->exporter_detail);
 ...
 $port_discharge			= $invoicedata->port_of_discharge;   		
 ?>	
<style>
.profile-pic {
	position: relative;
...
function view_pdf(no)
{
	if(no==1)
	{
		window.open(root+"view_bldraft/viewpdf", '_blank');
	}
	else if(no==2)
	{
		window.open(root+"view_bldraft/viewdoc", '_blank');
	}
	else{
		window.location= root+"view_bldraft/viewpdf";
	}
					...
					<div class="row">
						<div class="col-sm-12">
							...
							<div class="page-header title1">
									...
									<div class="pull-right form-group">
										...
										<a class="btn btn-primary tooltips" data-title="View in Pdf" href="javascript:;" onclick="view_pdf(2);" ><i class="fa fa-file"></i> Export Doc</a>
									</div>
							</div>
						</div>
					</div>
			 		<div class="row">
							...
							<div class="panel-default">
								...
								else
								{
									?>
											...
											<table cellspacing="0" cellpadding="0"   style="border:1px solid #333;padding: 5px;" class="pdf_class invoice_edit_cls"  width="100%">
												<tr>
													<th width="20%">Container Nos.Marks & Numbers</th>
													<th  width="60%">Number and Kind of Packages <br>Description of Goods
													</th>
													<th  width="15%"> </th>
									...
									<?php
											...
											$series_name_array 	= array();
												...
												for($i=0; $i<count($product_data);$i++)
												{
													...
													$Total_pallets  = floatval($product_data[$i]->total_ori_pallet);
													$sample_str = '';	  
													 $series_detail =  $product_data[$i]->custdescriptiongoods.'<br> HSN Code - '.$product_data[$i]->custhsnccode;
												 	...
												 	if(!in_array($product_data[$i]->con_entry,$con_array))
													{
													 ...
													 ?>
														<?=$product_data[$i]->container_no?><br>
												<?php
												 array_push($con_array,$product_data[$i]->con_entry);
													...
													$count = explode(",",$product_data[$i]->export_make_pallet_no);
														...
														if(!empty($product_data[$i]->pallet_ids) || $product_data[$i]->make_pallet_no == 1)
														{
															...
															$Grand_Total_pallet1 += $product_data[$i]->make_pallet_no;
															}
														}
														else if(!empty($product_data[$i]->export_make_pallet_no))
														{
															 ...
															 '<td style="text-align:center" rowspan="'.$export_rowspan.'">
																	...
																	$Grand_Total_pallet1 += $product_data[$i]->export_half_pallet;
														}
														else if(!empty($product_data[$i]->production_mst_id) || empty($product_data[$i]->pallet_row))
														{
															if(empty($product_data[$i]->export_half_pallet))
															{
															 '  <td style="text-align:center" > ';
																...
																{
																	 $product_data[$i]->origanal_pallet;
																	$Grand_Total_pallet1  += $product_data[$i]->origanal_pallet;
																}
																else if($product_data[$i]->orginal_no_of_big_pallet > 0 || $product_data[$i]->orginal_no_of_small_pallet > 0)
																{
																	 $product_data[$i]->orginal_no_of_big_pallet."<br>".$product_data[$i]->orginal_no_of_small_pallet;
																	$Grand_Total_pallet1  += $product_data[$i]->orginal_no_of_big_pallet;
																	$Grand_Total_pallet1  += $product_data[$i]->orginal_no_of_small_pallet;
																}
																else 
																{
																	 "-";
											...
											$con_array= array();
												...
												?>
													...
													<br>
															...
															<br>
															<?php 
															if(!empty($invoicedata->s_bill_no))
															{
																?>
																S.BILL NO.: <?=$invoicedata->s_bill_no?> DATE: <?=date('d/m/Y',strtotime($invoicedata->s_date))?>
															<?php 
															}
																?>
															<br>
							 ...
							 <pagebreak />	
								<table  width="100%" cellspacing="0" cellpadding="0"  style="border:1px solid #333;padding: 5px;" class="pdf_class invoice_edit_cls">
									<tr>
										 
									 	<td style="font-weight:bold;text-align:center">Container No.</td>
										<td style="font-weight:bold;text-align:center">Seal No</td>
										<!--<td style="font-weight:bold;text-align:center">Self-Seal No.</td>-->
									 	<td style="font-weight:bold;text-align:center">Boxes</td>
									 	<td style="font-weight:bold;text-align:center">Pallet</td>
										<td style="font-weight:bold;text-align:center">Net Weight</td>
										<td style="font-weight:bold;text-align:center">Gross Weight</td>
									</tr>
										 <?php
										 	$Total_plts = 0;
											$Total_sqm = 0;
						...
						<tr style="font-weight:bold;">
							...
							<td style="text-align:center"><?= $Grand_Total_pallet ?></td>
							<td colspan="2"></td>
						</tr>
								...
								<?php
									 ...
									 $_SESSION['bldfraft_invoice_no'] = $invoicedata->customer_invoice_no;
									 $_SESSION['bl_draft_content'] = $output;
									  if($mode=="1")
									 {
...
function edit_invoice()
{
     ...
     $.ajax({ 
       type: "POST", 
       url: root+"customerinvoiceview/bl_html_update",
       data:
	   {
	   ...
	   {
			 unblock_page("success","BL DRAFT UPDATED"); 
