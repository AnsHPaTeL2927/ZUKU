											 	...
											 	if(!in_array(trim($product_data[$i]->series_name.'<br> HSN Code - '.$product_data[$i]->hsnc_code),$series_name_array))
												{
											  	 ...
											  	 $n = 1;
												 	...
												 	if(!in_array(trim($description_goods),$product_desc_array))
													{
														...
														$product_desc_array[trim($description_goods)]['net_weight_kg'] = ($product_data[$i]->origanal_boxes * $product_data[$i]->weight_per_box);
...
<?=$performa_date?></strong></td>
										...
										<table  width="100%" cellspacing="0" cellpadding="0" style="padding:5px" class="pdf_class invoice_edit_cls" contenteditable="false">
											...
											<tr>
											  ...
											  <tr>
												...
												<td style="text-align:center;font-weight:bold"> PRICE IN KG - 
												...
												for($p=0;$p<count($product_desc_array);$p++)
												{
											 	 ...
											 	 ?>
													  ...
													  ?>														
															...
															//}
															$rateperkg = number_format($product_desc_array[$product_desc_array[$p]]['amount']/$product_desc_array[$product_desc_array[$p]]['net_weight_kg'],2);
															$product_amt = $product_desc_array[$product_desc_array[$p]]['net_weight_kg'] * $rateperkg;
															...
															<td style="text-align:center">
																<?=$product_desc_array[$product_desc_array[$p]]['boxes'] * $product_desc_array[$product_desc_array[$p]]['weight_per_box']?>
															</td>
															<td style="text-align:center">
															<?=$currency_symbol?>
															<?=$rateperkg?>
															...
															<td style="text-align:center"><span style="text-align:center">
															  <?=$currency_symbol?>
                                                              <?=number_format($product_amt,2,'.','')?>
															</span></td>
														</tr>
											
											<?php
											$no_of_row -=1;
													$Total_ammount 	+= $product_amt;
													$Total_amt 		+= $product_amt;
						 ...
						 <tr>
											<td colspan="5" rowspan="2"   style="font-weight:bold;vertical-align:middle; text-align:center;border-top:0.5px solid #333;border-right:0.5px solid #333;">
											<?php
											if ($invoicedata->igst_status == 1) {
									 	   ...
									 	   <td  style="font-weight:bold;text-align:center;border-top:0.5px solid #333;border-right:0.5px solid #333;"><?=$Total_box; ?></td>
									 	    <td  style="font-weight:bold;text-align:center;border-top:0.5px solid #333;border-right:0.5px solid #333;"><?=number_format($Total_sqm,2,'.',''); ?></td>
...
&nbsp;Value </td>
									  	 ...
									  	 <tr>
												...
												<td style="border-right:0.5px solid #333;border-top:0.5px solid #333;text-align:center">SQM</td>
													<td style="border-right:0.5px solid #333;border-top:0.5px solid #333;text-align:center">TOTAL WEIGHT</td>
										  </tr>
										 <?php
										$Total_amt =($invoicedata->calculation_operator == 2)?$Total_amt - $invoicedata->certification_charge:$Total_amt + $invoicedata->certification_charge;  ?>
										 ...
										 <tr>
										  ...
										  ?>
										  <tr>
												<td  style="border-right:0.5px solid #333;border-top:0.5px solid #333;font-weight:bold;vertical-align:top;" colspan="8"><?=strtoupper(convertamonttousd($Total_amt,$invoicedata->currency_name))?> ONLY </td>
							  ...
							  <table  width="100%" cellspacing="0" cellpadding="0" style="border:none" class="pdf_class invoice_edit_cls2" contenteditable="false">
											...
											$sizearray = array();
											 $no_of_row = 15;
												...
												if(!in_array($product_data[$i]->size_type_cm.$product_data[$i]->container_no.$product_data[$i]->product_id,$sizearray))
												{	
											...
											$Total_box	+= 	$totalpackage;				
													...
													array_push($sizearray,$product_data[$i]->size_type_cm.$product_data[$i]->container_no.$product_data[$i]->product_id);
												}
												if(!in_array($product_data[$i]->con_entry,$conarray))
												{
												?>
                                       ...
                                       <?php
											$no_of_row--;
                                        	  ...
                                        	  <tr>
													    ...
													    <td style="text-align:center"><?=($Grand_Total_pallet == 0)?"-":$Grand_Total_pallet; ?></td>
