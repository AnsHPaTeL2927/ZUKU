								...
								<div class="panel-body">
									<form role="form" class="form-horizontal" action="javascript:;" method="post" enctype="multipart/form-data" name="payment_add" id="payment_add">
									 	<div class="form-group">
											...
											<div class="col-sm-12">
												<input type="text" placeholder="Payment mode" id="payment_mode" class="form-control" name="payment_mode" />
											</div>
										</div>
										...
										<div class="modal-dialog">
											<div class="modal-content">
												<div class="modal-header">
													...
													<h4 class="modal-title">How To Manage Payment Mode</h4>
												</div>
												<div class="modal-body">
													<iframe id="cartoonVideo" width="560" height="315" src="https://www.youtube.com/embed/FLHbNpR54H8?rel=0&autoplay=1"" frameborder="0" allowfullscreen></iframe>
												</div>
											</div>
										</div>
									</div>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
			 ...
			 <form class="form-horizontal askform" action="javascript:;"  method="post" name="edit_form" id="edit_form">
				<div class="modal-body">
					 <div class="form-group">
					 	...
					 	<div class="col-sm-12">
					 		<input type="text" placeholder="Payment Mode" id="edit_payment_mode" class="form-control" name="edit_payment_mode" value="" >
					 	</div>
					</div>
		 		   </div>
            <div class="modal-footer">
				...
				<input type="hidden" name="editpaymentmode" id="editpaymentmode" />
			 </form>
        </div>
    </div>
</div>
...
function load_data_table()
{
	datatable = $("#datatable").dataTable({
			...
			"iDisplayLength": 50,
			"sAjaxSource": root+'payment_mode/fetch_record/',
...
{
	main_delete(deleleid,'payment_mode/deleterecord','payment_mode')
}
...
$("#payment_add").submit(function(event) {
	...
	postData.append("mode","add");
	 ...
	 var url = root+'payment_mode/manage';
	$.ajax({
            type: "post",
            url: url,
            data: postData,
	...
	$.ajax({
            ...
            success: function(responseData) {
               console.log(responseData);
			    var obj= JSON.parse(responseData);
				$(".loader").hide();
				if(obj.res==1 || $("#edit_payment_mode").val() == obj.editpaymentmode)
			   {
				    $("#myModal").modal('hide');
...
function edit_payment_mode(id)
{
	block_page();
     $.ajax({ 
              type: "POST", 
              url: root+"payment_mode/fetchdata",
              data: {"id": id}, 
              success: function (response) { 
				   ...
				   $("#myModal").modal({
					 ...
					 $("#eid").val(id);
					 $("#edit_payment_mode").val(obj.payment_mode);
					 $("#editpaymentmode").val(obj.payment_mode);
			 	 	unblock_page("",""); 
                  }
