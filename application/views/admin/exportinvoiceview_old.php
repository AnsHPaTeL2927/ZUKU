          ...
          <div class="page-header title1">
            <h3>View Export Invoice
              <div class="pull-right form-group">
                <div class="col-sm-4">
                  <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      <li> <a class="tooltips" data-toggle="tooltip" data-title="View Invoice (Nepal Packing)" href="<?=base_url('exportinvoiceview/exportinvoice_invoice/'.$invoicedata->export_invoice_id)?>" ><i class="fa fa-file-text-o"></i> View Only Invoice (Velsaa)</a> </li>  
                      <li> <a class="tooltips" data-toggle="tooltip" data-title="View Invoice (Nepal Packing)" href="<?=base_url('exportinvoiceview/index_velsa/'.$invoicedata->export_invoice_id)?>" ><i class="fa fa-file-text-o"></i> View Only Packing (Velsaa)</a> </li>  
                ...
                <div class="col-sm-4"> <a class="btn btn-info tooltips" data-title="View in Pdf" href="javascript:;" onclick="view_pdf(1);"><i class="fa fa-file-pdf-o"></i> Export Pdf</a> 
                </div>	
                <div class="col-sm-4"> 	<a class="btn btn-warning tooltips" data-title="View in Excel" href="javascript:;"  onclick="toExcel();"   ><i class="fa fa-file"></i> Export Excel</a>
                ...
                <table  width="100%" cellspacing="0" cellpadding="0" style="padding:5px" class="pdf_class invoice_edit_cls" contenteditable="false">
                  ...
                  <tr>
                    ...
                    <td style="text-align:center" colspan="2"><?php
                      ...
                      <?=!empty($product_desc_array[$product_desc_array[$p]]['water_text'])?'<br>'.$product_desc_array[$product_desc_array[$p]]['water_text']:""?>						 <br />						<?php						if(!empty($product_desc_array[$product_desc_array[$p]]['company_name']))						{						?>						Manufacturer Name : <?=$product_desc_array[$product_desc_array[$p]]['company_name']?>						<?php						}						?>						 <br />						<?php						if(!empty($product_desc_array[$product_desc_array[$p]]['suppiler_invoice_no']))						{						?>						Invoice No : <?=$product_desc_array[$product_desc_array[$p]]['suppiler_invoice_no']?> &nbsp;&nbsp; 						<?php						}						?>						<?php						if($product_desc_array[$product_desc_array[$p]]['suppiler_invoice_date'] != '0000-00-00' && $product_desc_array[$product_desc_array[$p]]['suppiler_invoice_date'] != '1970-01-01')						{						?>						Date : <?=date('d/m/Y',strtotime($product_desc_array[$product_desc_array[$p]]['suppiler_invoice_date']))?>  												<?php						}						?>						
                      <?php
																}
																else
																{
              ...
              <div class="save_invoice_html1">
                ...
                <table  width="100%" cellspacing="0" cellpadding="0" style="border:none" class="pdf_class invoice_edit_cls1" contenteditable="false">
                  <tr>
                    ...
                    <td style="text-align:center;font-weight:bold">BOXES</td>
                    <td style="text-align:center;font-weight:bold">PALLETS</td>
                    <td style="text-align:center;font-weight:bold">Net By Size</td>
                    <!--<td style="text-align:center;font-weight:bold">Gross By Size</td>-->
											...
											$no_of_row = 15;
												...
												for($i=0; $i<count($product_data);$i++)
												{
												 	...
												 	if(!in_array($product_data[$i]->con_entry,$con_array))
													{
														...
														//$net_weight 	= ($product_data[$i]->origanal_boxes * $product_data[$i]->weight_per_box);
                  ...
                  <tr>
                    ...
                    <?php 
												...
												?>
                    <td style="text-align:center" >
					<?=var_dump(($product_data[$i]->weight_per_box * $product_data[$i]->origanal_boxes ));?>
					<?=$product_data[$i]->size_type_mm?></td>
											...
											// SIZE WISE NET AND GROSS WIGHT----------------------------------------------------------------------------------------------------//
															  echo '<td style="text-align:center;" >'.($product_data[$i]->weight_per_box * $product_data[$i]->origanal_boxes).'</td>';
																...
																// {
																	...
																	// {
																		// $pallet_ids = explode(",",$product_data[$i]->pallet_row);
																	// echo  '<td  style="text-align:center" class="text-center" >
																		// '.(($product_data[$i]->weight_per_box * $product_data[$i]->origanal_boxes) + ($product_data[$i]->make_pallet_no * $product_data[$i]->pallet_weight)).'     
																	// </td>';
																	// $Grand_Total_pallet += $product_data[$i]->make_pallet_no;
																	// }
																// }
																...
																// {
																		// $export_rowspan = explode(",",$product_data[$i]->export_make_pallet_no);
																	// echo  '<td  style="text-align:center" class="text-center" >
																		// '.(($product_data[$i]->weight_per_box * $product_data[$i]->origanal_boxes) + ($product_data[$i]->export_half_pallet * $product_data[$i]->pallet_weight)).'     
																	// </td>';
																	// $Grand_Total_pallet += $product_data[$i]->export_half_pallet;
																// }
																// else if(empty($product_data[$i]->pallet_row) && empty($product_data[$i]->export_half_pallet))
																// {
																	// echo '  <td style="text-align:center" > ';
																		// if($product_data[$i]->origanal_pallet > 0)
																		// {
																			// echo (($product_data[$i]->weight_per_box * $product_data[$i]->origanal_boxes) + ($product_data[$i]->origanal_pallet * $product_data[$i]->pallet_weight));
																			// $Grand_Total_pallet  += $product_data[$i]->origanal_pallet;
																		// }
																		// else if($product_data[$i]->orginal_no_of_big_pallet > 0 || $product_data[$i]->orginal_no_of_small_pallet > 0)
																		// {
																			// echo (($product_data[$i]->weight_per_box * $product_data[$i]->origanal_boxes) + ($product_data[$i]->orginal_no_of_big_pallet * $product_data[$i]->pallet_weight))."<br>".(($product_data[$i]->weight_per_box * $product_data[$i]->origanal_boxes) + ($product_data[$i]->orginal_no_of_small_pallet * $product_data[$i]->pallet_weight));
																			// $Grand_Total_pallet  += $product_data[$i]->no_of_big_pallet;
																			// $Grand_Total_pallet  += $product_data[$i]->orginal_no_of_small_pallet;
																		// }
																		// else 
                   ...
                   <td style="text-align:center;" rowspan="<?=$rowcon_no?>" ><?=number_format($net_weight,2,'.','')?></td>
                    <!--<td style="text-align:center;" rowspan="<?=$rowcon_no?>"><?=($product_data[$i]->weight_per_box * $product_data[$i]->origanal_boxes);?></td>-->
