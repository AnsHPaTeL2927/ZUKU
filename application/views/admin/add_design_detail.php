						...
						<div class="col-sm-12">
							...
							<h3>
								...
								<div class="pull-right">
									<a href="<?=base_url().'add_design_detail/download/'.$cust_data->id?>" target="_blank" class="btn btn-info" >
							...
							<div class="col-md-4">
										...
										<select class="select2" name="filter_product_id" id="filter_product_id" required title="Select Product" onchange="load_data()" >
												...
												foreach($get_product_name_data as $product_row)
												{
													...
													$sel = '';
													 if($product_row->product_id == $_SESSION['customer_product_id'])
													 {
							 ...
							 <a href="<?=base_url().'add_customer_detail/index/'.$cust_data->id?>" class="btn btn-primary pull-right" > Next </a>
						</div>
					</div>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content"  style="max-height: 600px;overflow-y: auto;">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Import Price for <?=$cust_data->c_companyname?>  </h4>
            </div>
...
function load_data()
{
	block_page();
      $.ajax({ 
            type: "POST", 
            url:  root+"add_design_detail/load_data",
            data: {
...
function update_price_by_product(product_id,no)
{
       ...
       block_page();
	    ...
	    $.ajax({ 
              type: "POST", 
              url:  root+"add_design_detail/update_multiple_price",
              data: {
					"cust_id"			: <?=$cust_data->id?>,
...
$("#import_form").submit(function(event) {
	...
	var url = root+'add_design_detail/import_price'; 
	$.ajax({
            type: "post",
            url: url,
            data: postData,
...
function update_price(finish_id,product_id,n,design_rate_id)
{
       ...
       block_page();
        ...
        $.ajax({ 
              type: "POST", 
              url:  root+"add_design_detail/updateprice",
              data: {
					"cust_id"			: <?=$cust_data->id?>,
					"finish_id"			: finish_id,
