				...
				<div class="col-sm-12">
					<div class=" panel-default">
										...
										<?php
											...
											?>
												<tr>
													...
													<td>
														<?=($product_data[$i]->product_id == 0)?"-":number_format($one_container/100,2)?>
													</td>
													<td>
														<?php
														$total_container += ($product_data[$i]->product_id == 0)?0:number_format($one_container/100,2);
														...
														<div class="dropdown">
															...
															<span class="caret"></span></button>
															  <ul class="dropdown-menu">
																<li>
																	<a class="tooltips" data-title="Edit"  onclick="edit_product(<?=$product_data[$i]->estimate_trn_id?>,<?=$deletestatus?>,0,<?=(!empty($product_data[$i]->product_id))?'0':'1'?>,<?=$packing_row->estimate_packing_id?>)" href="javascript:;" ><i class="fa fa-pencil"></i> Edit</a>
																</li>
																<?php
																if($deletestatus != 1)
																{
																?>
																	<li>
																		<a class="tooltips" data-title="Delete"  onclick="delete_product(<?=$product_data[$i]->estimate_trn_id?>)" href="javascript:;" ><i class="fa fa-trash"></i> Delete</a>
																	</li>
																 <?php 
																 }
																 else{?>
										 ...
										 <tr>
											 ...
											 <?php 
												...
												<tr>
													...
													<th class="text-center"><?=$Total_sqf; ?></th>
												...
												<tr>
												 	<th colspan="7"> 
														<span style="width:20%;float: left;" > INVOICE VALUE SAY </span>
														
														<input id="invoicevalue_say" type="text"  name="invoicevalue_say" placeholder="FOB" value="<?=$invoicevalue_say?>" class="form-control col-md-2" style="width:10%;margin-right:10px;"  /> 

														<span style="width:70%;"> <?=convertamonttousd($Total_ammount,$invoicedata->currency_name);?>	ONLY</span>
													</th>
						...
						<div style="padding: 14px;padding-left:0px;">
							...
							<input type="hidden" id="performainvoiceid" name="performainvoiceid" value="<?=$invoicedata->estimate_id?>"> 
    ...
    <div class="modal-dialog modal-lg" style="width: 1266px;overflow-x: scroll;">
        ...
        <div class="modal-content"  style="max-height: 600px;overflow-y: auto;">
            ...
            <div class="modal-body">
                <div class="row">
					 <input type="hidden" id="estimateid" name="estimateid" value="<?=$invoicedata->estimate_id?>"> 
					<div class="col-md-12">					
			...
			<input type="hidden" name="estimate_trn_id" id="estimate_trn_id"/>
...
<div id="wallproduct" class="modal fade" role="dialog">
    <div class="modal-dialog modal-lg" style="width:1400px">
        ...
        <div class="modal-content"  style="max-height: 600px;overflow-y: auto;">
            ...
            <div class="modal-body">
                <div class="row">
					 <input type="hidden" id="wallestimate_id" name="wallestimate_id" value="<?=$invoicedata->estimate_id?>"> 
					   ...
					   <div class="field-group" >
							<table class="table table-bordered table-hover walltable">
								<tr>
									...
									<td style="text-align:center" width="7%" class="">Pallet </td>
									<td style="text-align:center" width="7%">Boxes</td>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content"  style="max-height: 600px;overflow-y: auto;">
			...
			<form class="form-horizontal askform" action="javascript:;"  method="post" name="import_form" id="import_form">
			 ...
			 <input type="hidden" id="importestimate_id" name="importestimate_id" value="<?=$invoicedata->estimate_id?>"> 
            <div class="modal-body">
                <div class="row">
...
<div id="otherproduct" class="modal fade" role="dialog">
    <div class="modal-dialog modal-lg" style="width:1400px">
        ...
        <div class="modal-content"  style="max-height: 600px;overflow-y: auto;">
            ...
            <div class="modal-body">
                <div class="row">
					 <input type="hidden" id="otherestimate_id" name="otherestimate_id" value="<?=$invoicedata->estimate_id?>"> 
					 <input type="hidden" id="otherestimate_packing_id" name="otherestimate_packing_id" value=""> 
					 <input type="hidden" id="otherestimate_trn_id" name="otherestimate_trn_id" value=""> 
					<div class="col-md-12 product_html">					
					   <div class="field-group" >
...
$("#other_product_form").submit(function(event) {
	...
	$.ajax({
            type: "post",
            url:  root+'quotation_product/othermanage',
            data: postData,
			processData: false,
			contentType: false,
			cache: false,
            success: function(responseData) {
			  ...
			  if(obj.res==1)
			   {
				   $("#otherproduct").trigger('reset');
				    unblock_page("success","Sucessfully Inserted.");
					setTimeout(function(){ window.location=root+'quotation_product/index/'+<?=$invoicedata->estimate_id?> },1000);
...
$("#import_form").submit(function(event) {
	...
	block_page();
	var postData= new FormData(this);
	var url = root+'quotation_product/import_order';
            ...
            success: function(responseData) {
               console.log(responseData);
				...
				if(obj.res==1)
			    {
				   ...
				   setTimeout(function(){ window.location=root+'product/index/'+<?=$invoicedata->estimate_id?> },1000);
				...
				else if(obj.res==4)
				{
					...
					setTimeout(function(){ window.location=root+'excel_error/index/'+<?=$invoicedata->estimate_id?> },1000);
		 		}
				else if(obj.res==0)
				{
...
$("#wallproduct_form").submit(function(event) {
	...
	$.ajax({
            type: "post",
            url:  root+'quotation_product/wallmanage',
            data: postData,
			processData: false,
			contentType: false,
			cache: false,
 ...
 function add_wall_row()
{
	 $.ajax({ 
       type: "POST", 
       url: root+"quotation_product/add_wall_design_row",
       data: {
...
{  
	if(product_id != "")
	{
		...
		$.ajax({ 
		type: "POST", 
		url: root+"quotation_product/load_design_data",
		data: {
				"id"				: product_id,
				"mode" 				: mode,
				"estimate_trn_id" 	: $("#estimate_trn_id").val(),
				"customer_id" 		: <?=$invoicedata->consigne_id?>,
...
if(product_size_id != "")
{
     ...
     $.ajax({ 
       type: "POST", 
       url: root+"quotation_product/load_packing",
       data: {
...
function add_row()
{
	 $.ajax({ 
       type: "POST", 
       url: root+"quotation_product/add_design_row",
       data: {
...
function check_product(step)
{
	 if(<?=count($product_data)?> == 0)
	{
		...
		update_calc(step,0);
		unblock_page("success","Sucessfully Updated.");
		  setTimeout(function(){ window.location='<?php echo base_url();?>quotation_pdf/index/<?=$invoicedata->estimate_id ?>'; },1000);
		 
	}
}
function delete_product(estimate_trn_id)
{
  ...
  confirmButtonText: 'Yes, delete it!'
		 ...
		 if (result.value) {
			block_page();
			  $.ajax({ 
              type: "POST", 
              url: root+'quotation_product/deleterecord',
              data: {
                "id": estimate_trn_id
              }, 
              cache: false, 
              success: function (data) { 
				...
				if(obj.res==1)
				{
					updateinvoice(<?=$invoicedata->estimate_id?>);
					unblock_page('success',"Record Successfully Deleted");
					 setTimeout(function(){ window.location=root+'quotation_product/index/<?=$invoicedata->estimate_id?>'; },1500);
				}
                else{
  ...
  confirmButtonText: 'Yes, delete it!'
	 ...
	 if (result.value) {
			block_page();
              ...
              data: {
                "id": id,
				"invoice_id" : <?=$invoicedata->estimate_id?>
              }, 
              cache: false, 
              success: function (data) { 
				var obj = JSON.parse(data);
				if(obj.res==1)
				{
					updateinvoice(<?=$invoicedata->estimate_id?>);
					unblock_page('success',"Record Successfully Deleted");
					setTimeout(function(){ window.location=root+'product/index/<?=$invoicedata->estimate_id?>'; },1500);
				}
                else{
					unblock_page('error',"Somthing Wrong.")
				}
...
function update_calc(step,val)
{
	 block_page();
	   $.ajax({ 
             type: "POST",
			 async: false,			 
             url: root+"quotation_product/update_quotation",
             data: {
	...
	for (var i = 0; i <inps.length; i++) 
	{
	 ...
	 block_page();
		var postData= new FormData(this);
		 	$.ajax({
            type: "post",
            url:  root+'quotation_product/manage',
            data: postData,
			processData: false,
			contentType: false,
			cache: false,
...
function edit_product(estimate_trn_id,deletestatus,check_production_sheet,other_status,estimate_packing_id)
{
	 
	block_page();
     $.ajax({ 
              type: "POST", 
              url: root+"quotation_product/fetchproductdata",
              data: {
					"id" 					: estimate_trn_id,
					"estimate_packing_id" 	: estimate_packing_id,
			  ...
			  { 
				  ...
				  if(other_status  == 1)
				  {
					 ...
					 $("#otherestimate_packing_id").val(obj.packing.estimate_packing_id)
					 $("#otherestimate_trn_id").val(obj.estimate_trn_id)
						var  other_qty = 0;
				  ...
				  else
				  {
						...
						$("#estimate_trn_id").val(estimate_trn_id);
